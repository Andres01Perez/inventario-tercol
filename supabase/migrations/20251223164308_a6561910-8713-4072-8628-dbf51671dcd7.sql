-- =====================================================
-- MIGRACIÓN: Validación Individual por Ubicación
-- =====================================================

-- 1. Agregar columnas de validación a locations
ALTER TABLE public.locations 
ADD COLUMN IF NOT EXISTS validated_at_round INTEGER NULL,
ADD COLUMN IF NOT EXISTS validated_quantity NUMERIC NULL;

-- Comentarios para documentar
COMMENT ON COLUMN public.locations.validated_at_round IS 'Ronda en la que esta ubicación individual fue validada (coincidió con un conteo previo)';
COMMENT ON COLUMN public.locations.validated_quantity IS 'Cantidad validada para esta ubicación específica';

-- 2. Crear función auxiliar para verificar si una ubicación está validada
CREATE OR REPLACE FUNCTION public.is_location_validated(_location_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  SELECT validated_at_round IS NOT NULL 
  FROM public.locations 
  WHERE id = _location_id
$$;

-- 3. Reemplazar función validate_and_close_round con nueva lógica de validación por ubicación
...
BEGIN
  ...
  -- Iterar sobre cada ubicación de esta referencia
  FOR v_location IN 
    SELECT l.id, l.location_name, l.validated_at_round, l.validated_quantity
    FROM locations l 
  ...
  LOOP
    ...
    IF v_master.audit_round = 1 THEN
      -- Verificar que tenga ambos conteos
