-- Update validate_and_close_round to handle discovered_at_round
...
BEGIN
  ...
  -- ==============================================================================
  -- FASE 0: Verificar sumas totales primero (antes de validar ubicaciones individuales)
  -- ==============================================================================
  IF v_master.audit_round = 1 THEN
    DECLARE
      ...
      v_sum_c2 NUMERIC;
    BEGIN
      -- Calcular suma de C1 y C2 para todas las ubicaciones
      ...
      -- Si SUM(C1) = ERP o SUM(C2) = ERP o SUM(C1) = SUM(C2), cerrar directamente
      IF (v_sum_c1 = v_erp AND v_sum_c1 > 0) OR (v_sum_c2 = v_erp AND v_sum_c2 > 0) OR (v_sum_c1 = v_sum_c2 AND v_sum_c1 > 0) THEN
        -- Validar todas las ubicaciones con C2 (o C1 si C2 no existe)
        FOR v_location IN 
          SELECT l.id FROM locations l WHERE l.master_reference = _reference AND l.validated_at_round IS NULL
        LOOP
          ...
          UPDATE locations SET 
            validated_at_round = CASE WHEN v_count_c2 IS NOT NULL THEN 2 ELSE 1 END,
            validated_quantity = COALESCE(v_count_c2, v_count_c1)
          WHERE id = v_location.id;
        END LOOP;
        
        -- Calcular suma total validada
        SELECT COALESCE(SUM(validated_quantity), 0) INTO v_sum_validated FROM locations WHERE master_reference = _reference;
        ...
        VALUES (_reference, _admin_id, 'closed_by_total_sum', v_master.audit_round, jsonb_build_object('sum', v_sum_validated));
        
        RETURN jsonb_build_object('success', true, 'action', 'closed', 'reason', 'total_sum_match', 'total', v_sum_validated);
      END IF;
    END;
  END IF;

  -- ==============================================================================
  -- NUEVA LÓGICA: Validar cada ubicación INDIVIDUALMENTE
  -- ==============================================================================
  ...
  LOOP
    ...
    IF v_master.audit_round = 1 THEN
      ...
      IF v_discovered_round = 2 THEN
        IF v_count_c2 IS NOT NULL THEN
          -- Ubicación descubierta en C2, se valida automáticamente con C2
    ...
    -- ==============================================================================
    -- CONTEO 3: Verificar C3 contra C1 o C2 (o solo C2 si discovered_at_round = 2)
    -- ==============================================================================
    ...
    ELSIF v_master.audit_round = 5 THEN
      ...
      -- En C5, el superadmin valida forzadamente
  ...
  IF v_all_validated AND v_validated_locations > 0 THEN
    -- Verificar si coincide con ERP para determinar status final
  ...
  -- Si hay ubicaciones pendientes, determinar siguiente acción
  ELSIF v_pending_locations > 0 THEN
    IF v_master.audit_round = 1 THEN
      -- Después de C1/C2: Si hay pendientes, pasan a C3
    ...
    ELSIF v_master.audit_round = 3 THEN
      -- Después de C3: Si hay pendientes, pasan a C4
    ...
    ELSIF v_master.audit_round = 4 THEN
      -- Después de C4: Si hay pendientes, pasan a C5 (crítico)
