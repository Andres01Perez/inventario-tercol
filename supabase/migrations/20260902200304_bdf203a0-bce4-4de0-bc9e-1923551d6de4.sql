...
BEGIN
  ...
  -- Cierres reales: nunca se reabren
  IF v_status IN ('auditado', 'cerrado_forzado') THEN
    RETURN jsonb_build_object('success', true, 'action', 'skipped', 'reason', 'bloque_cerrado', 'status', v_status);
  END IF;

  IF v_status IN ('n/a', 'critico') THEN
    SELECT COUNT(*) INTO v_open_locs
    FROM locations l
  ...
  IF v_round = 1 THEN
    IF v_sum_c1 = v_erp AND (v_sum_c1 > 0 OR v_zero_ok) THEN v_matched_round := 'C1=ERP'; v_matched_total := v_sum_c1;
    ELSIF v_sum_c2 = v_erp AND (v_sum_c2 > 0 OR v_zero_ok) THEN v_matched_round := 'C2=ERP'; v_matched_total := v_sum_c2;
    ELSIF v_sum_c1 = v_sum_c2 AND (v_sum_c1 > 0 OR v_zero_ok) THEN v_matched_round := 'C1=C2'; v_matched_total := v_sum_c1;
    END IF;
  ELSIF v_round = 3 THEN
    IF v_sum_c3 = v_erp AND (v_sum_c3 > 0 OR v_zero_ok) THEN v_matched_round := 'C3=ERP'; v_matched_total := v_sum_c3;
    ELSIF v_sum_c3 = v_sum_c1 AND (v_sum_c3 > 0 OR v_zero_ok) THEN v_matched_round := 'C3=C1'; v_matched_total := v_sum_c3;
    ELSIF v_sum_c3 = v_sum_c2 AND (v_sum_c3 > 0 OR v_zero_ok) THEN v_matched_round := 'C3=C2'; v_matched_total := v_sum_c3;
    END IF;
  ELSIF v_round = 4 THEN
    IF v_sum_c4 = v_erp AND (v_sum_c4 > 0 OR v_zero_ok) THEN v_matched_round := 'C4=ERP'; v_matched_total := v_sum_c4;
    ELSIF v_sum_c4 = v_sum_c3 AND (v_sum_c4 > 0 OR v_zero_ok) THEN v_matched_round := 'C4=C3'; v_matched_total := v_sum_c4;
    ELSIF v_sum_c4 = v_sum_c2 AND (v_sum_c4 > 0 OR v_zero_ok) THEN v_matched_round := 'C4=C2'; v_matched_total := v_sum_c4;
    ELSIF v_sum_c4 = v_sum_c1 AND (v_sum_c4 > 0 OR v_zero_ok) THEN v_matched_round := 'C4=C1'; v_matched_total := v_sum_c4;
    END IF;
  END IF;
  ...
  LOOP
    ...
    IF v_round = 1 THEN
      IF v_discovered = 2 THEN
        ...
        END IF;
      ELSIF v_count_c1 IS NOT NULL AND v_count_c2 IS NOT NULL AND v_count_c1 = v_count_c2 AND (v_count_c1 > 0 OR v_zero_ok) THEN
    ...
    ELSIF v_round = 3 THEN
      IF v_count_c3 IS NOT NULL THEN
        ...
        ELSIF v_discovered = 2 THEN
          IF v_count_c3 = v_count_c2 AND (v_count_c3 > 0 OR v_zero_ok) THEN
            v_location_quantity := v_count_c3; v_location_round := 3; v_location_reason := v_prefix || 'C3=C2 (discovered_at_C2)';
          END IF;
        ELSIF (v_count_c3 = v_count_c1 OR v_count_c3 = v_count_c2) AND (v_count_c3 > 0 OR v_zero_ok) THEN
    ...
    ELSIF v_round = 4 THEN
      IF v_count_c4 IS NOT NULL THEN
        ...
        ELSIF v_discovered = 3 THEN
          IF v_count_c4 = v_count_c3 AND (v_count_c4 > 0 OR v_zero_ok) THEN
            v_location_quantity := v_count_c4; v_location_round := 4; v_location_reason := v_prefix || 'C4=C3 (discovered_at_C3)';
          END IF;
        ELSIF v_discovered = 2 THEN
          IF (v_count_c4 = v_count_c2 OR v_count_c4 = v_count_c3) AND (v_count_c4 > 0 OR v_zero_ok) THEN
          ...
          END IF;
        ELSIF (v_count_c4 = v_count_c1 OR v_count_c4 = v_count_c2 OR v_count_c4 = v_count_c3) AND (v_count_c4 > 0 OR v_zero_ok) THEN
          v_location_quantity := v_count_c4; v_location_round := 4;
...
-- Protección: no permitir conteos nuevos en ubicaciones ya validadas (bloque cerrado)
...
DROP TRIGGER IF EXISTS block_count_on_validated_trg ON public.inventory_counts;
CREATE TRIGGER block_count_on_validated_trg
BEFORE INSERT OR UPDATE ON public.inventory_counts
FOR EACH ROW EXECUTE FUNCTION public.block_count_on_validated_location();